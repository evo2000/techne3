...
/**
 * Vendored copy of gajus/dindent (BSD-3-Clause).
 *
...
class Indenter {
    private
        ...
        $temporary_replacements_comment = array(),
     ...
     * @param int $type ELEMENT_TYPE_BLOCK|ELEMENT_TYPE_INLINE
     * @return void
     */
    ...
    public function indent ($input) {
        ...
        // Patched: dindent does not preserve HTML comments by default, which
        // corrupts multi-line content like ASCII art (whitespace collapsed,
        // lines merged). Preserve comments the same way <script> is preserved.
        if (preg_match_all('/<!--[\s\S]*?-->/', $input, $matches)) {
            $this->temporary_replacements_comment = $matches[0];

            foreach ($matches[0] as $i => $match) {
                $input = str_replace($match, '<!--DINDENT_COMMENT_' . ($i + 1) . '-->', $input);
            }
        }
        ...
        do {
            ...
            $patterns = array(
                // block tag
                '/^(<([a-zA-Z0-9]+)(?:[^>]*)>(?:[^<]*)<\/(?:\2)>)/' => static::MATCH_INDENT_NO,
                // DOCTYPE
        ...
        foreach ($this->temporary_replacements_comment as $i => $original) {
            $output = str_replace('<!--DINDENT_COMMENT_' . ($i + 1) . '-->', $original, $output);
        }
